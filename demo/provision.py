...
class WebVhManager:
    def __init__(self):
        ...
        self.agent_admin_url = os.getenv("AGENT_ADMIN_API_URL", None)
        self.agent_admin_headers = {"X-API-KEY": os.getenv("AGENT_ADMIN_API_KEY", "")}
    
    def configure(self):
        r = requests.post(
            f"{self.agent_admin_url}/did/webvh/configuration", 
            json={
                "auto_attest": True,
                "endorsement": False,
    ...
    def create_did(self, ns: str, alias: str):
        alias = alias.lower().replace(" ", "-")
        r = requests.get(f"{self.server_url}?ns={ns}&alias={alias}")
        print(r.json())
...
webvh.create_did("pilots", "Chief Permitting Officer")
# webvh.create_did("pilots", "did-01")
# webvh.create_did("pilots", "did-01")
