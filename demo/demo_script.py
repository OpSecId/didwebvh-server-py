...
# UNTP Schema and Context URLs (version 0.6.0)
UNTP_SCHEMAS = {
...
async def fetch_and_cache_schema(client: httpx.AsyncClient, credential_type: str) -> dict:
    """Fetch UNTP schema from URL and cache it."""
    if credential_type in _schema_cache:
        return _schema_cache[credential_type]
    
    schema_url = UNTP_SCHEMAS.get(credential_type)
    if not schema_url:
        raise ValueError(f"No schema URL for credential type: {credential_type}")
    
    logger.info(f"  Fetching schema: {credential_type}")
    response = await client.get(schema_url, follow_redirects=True)
    response.raise_for_status()
    
    schema = response.json()
    _schema_cache[credential_type] = schema
    return schema


async def validate_credential_schema(client: httpx.AsyncClient, credential: dict, credential_name: str) -> bool:
    """Validate a credential against its UNTP JSON schema."""
    # Determine credential type
    cred_types = credential.get("type", [])
    schema_type = None
    
    if "DigitalConformityCredential" in cred_types:
        schema_type = "DigitalConformityCredential"
    elif "DigitalIdentityAnchor" in cred_types:
        schema_type = "DigitalIdentityAnchor"
    elif "DigitalProductPassport" in cred_types:
        schema_type = "DigitalProductPassport"
    else:
        logger.warning(f"  ⚠️  No UNTP schema for {credential_name} (type: {cred_types})")
        return True  # Skip validation for non-UNTP credentials
    
    try:
        # Fetch schema
        schema = await fetch_and_cache_schema(client, schema_type)
        ...
        if errors:
            logger.error(f"  ❌ Schema validation FAILED for {credential_name}")
            for i, error in enumerate(errors[:3], 1):  # Show first 3 errors
                path = ' → '.join(str(p) for p in error.path) if error.path else 'root'
                logger.error(f"    Error {i}: {path}: {error.message}")
            if len(errors) > 3:
                logger.error(f"    ... and {len(errors) - 3} more error(s)")
            return False
        else:
        ...
        logger.error(f"  ❌ Validation error for {credential_name}: {e}")
        return False
...
async def create_did(client: httpx.AsyncClient, server_url: str, actor: dict, output_dir: Path):
    ...
    # Build domain path using the actual server domain
    from urllib.parse import urlparse
    parsed_url = urlparse(server_url)
    server_domain = parsed_url.netloc  # e.g., "localhost:8000" or "didwebvh.example.com"
    domain_path = f"{server_domain}/{namespace}/{alias}"
...
async def issue_mines_act_permit(client: httpx.AsyncClient, server_url: str, cpo: dict, tek: dict, cpo_status_list: dict):
    ...
    # Create credential as plain dictionary (avoid Pydantic type fields that don't exist in DCC context)
    # Use did:web for issuer (credentials are signed with did:web)
    credential = {
        "@context": [
        ...
        "credentialStatus": {
            ...
            "statusPurpose": "revocation",
            "statusListIndex": str(random.randint(0, 100000)),
            ...
            "authorisation": [
                {
                    ...
                    "endorsementEvidence": {
                        "linkURL": "https://www.bclaws.gov.bc.ca/civix/document/id/complete/statreg/96293_01",
                        "linkName": "Mines Act [RSBC 1996] CHAPTER 293",
                        "linkType": "https://test.uncefact.org/vocabulary/linkTypes/regulatoryDocument"
                    }
                }
            ],
            "assessment": [
                {
                    ...
                    "id": "https://permits.nrs.gov.bc.ca/permits/M-24-0001-COPPER",
                    "name": "Copper Mining Compliance Assessment - Permit M-24-0001-COPPER",
                    "description": "Assessment of compliance with BC Mines Act for copper mining operations under permit M-24-0001-COPPER",
                    ...
                    "assessedFacility": [
                        {
                            "facility": {
                                "type": ["Facility"],
                                "id": "https://mines.nrs.gov.bc.ca/facilities/MINE-BC-001",
                                "name": "Tek Copper Mine",
                                "registeredId": "MINE-BC-001",
                                "idScheme": {
                                    "type": ["IdentifierScheme"],
                                    "id": "https://mines.nrs.gov.bc.ca/",
                                    "name": "BC Mines Digital Services"
                                }
                            },
    ...
    # Validate against UNTP schema before signing
    is_valid = await validate_credential_schema(client, credential_dict, "Mines Act Permit (DCC)")
    if not is_valid:
        raise ValueError("Credential failed schema validation")
...
async def issue_bc_registration(client: httpx.AsyncClient, server_url: str, roc: dict, tek: dict, roc_status_list: dict):
    ...
    # Validate against UNTP schema before signing
    is_valid = await validate_credential_schema(client, credential_dict, "BC Registration (DIA)")
    if not is_valid:
        raise ValueError("Credential failed schema validation")
    ...
    # Generate sqid for the DIA credential
    sqids = Sqids(min_length=8)
    timestamp_ms = int(time.time() * 1000)
    dia_sqid = sqids.encode([timestamp_ms])
    
    # Publish the DIA credential to the server
    response = await client.post(
        f"{server_url}/{roc['namespace']}/{roc['alias']}/credentials",
        json={
            "verifiableCredential": enveloped_vc,
            "options": {
                "credentialId": dia_sqid
            }
        }
    )
    
    if response.status_code != 201:
        logger.error(f"  ✗ Failed to publish DIA ({response.status_code})")
        response.raise_for_status()
    
    logger.info(f"  ✓ Published ({dia_sqid})")
...
async def issue_product_passport(client: httpx.AsyncClient, server_url: str, tek: dict, cpo: dict, mines_permit_sqid: str, tek_status_list: dict):
    ...
    # Construct the resolvable DCC URL (use HTTPS URL)
    permit_credential_url = f"{server_url}/{cpo['namespace']}/{cpo['alias']}/credentials/{mines_permit_sqid}"
    ...
    credential = {
        ...
        "credentialSubject": {
            ...
            "name": "Copper Concentrate Product Passport",
            "product": {
                "type": ["Product"],
                "id": "urn:uuid:product-copper-batch-001",
            ...
            "conformityClaim": [
                {
                    ...
                    "id": "urn:uuid:claim-copper-compliance-001",
                    "description": "Compliance with BC Mines Act requirements for copper mining operations",
                    "conformityTopic": "governance.compliance",
                    "conformance": True,
                    "assessmentDate": "2024-12-19",
                    "conformityEvidence": {
                        "linkURL": permit_credential_url,
                        "linkName": "BC Mines Act Permit",
    ...
    # Validate against UNTP schema before signing
    is_valid = await validate_credential_schema(client, credential, "Product Passport (DPP)")
    if not is_valid:
        raise ValueError("Credential failed schema validation")
...
async def run_demo(server_url: str):
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        bc_registration = await issue_bc_registration(client, server_url, roc, tek, roc_status)
